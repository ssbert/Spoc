    ...
    function init(id) {
        ...
        this.initPage = function () {
            if (!guidIsEmpty(id)) {
                ...
                $("#policyItem-container").hide();
            }
        }

        function loadData() {
            VE.Mask("");
            var url = webApi + "/ExamPolicy/Get?id=" + id;
...
var PolicyNodeListClass = (function () {
    function init(policyNode, policyItemList) {
        ...
        var handle = evtBus.addEvt("show_policy_node", function (id) {
            ...
            $("#policyItem-container").show();
            $("#policyNode-dg").datagrid("resize");
            $("#policyItemList-dg").datagrid("resize");
            isShow = true;
            paramCache.policyUid = id;
            loadData(paramCache);
            policyItemList.query();
        });
        ...
        this.del = function (index) {
            ...
            $.messager.confirm("删除确认", "确定进行删除操作吗？", function (b) {
                ...
                var url = webApi + "/ExamPolicyNode/Delete?ids=" + idArray.join(",");
                nv.get(url, function (data) {
        ...
        function loadData(param) {
            $("#policyNode-dg").datagrid("loading");
            var url = webApi + "/ExamPolicyNode/GetList?policyId=" + paramCache.policyUid;
            nv.get(url, function (data) {
                $("#policyNode-dg").datagrid("loaded");
                if (data.success) {
                    $("#policyNode-dg").datagrid("loadData", data.result);
                    policyItemList.policyNodeFormAddData(data.result);
                } else {
                    $.messager.alert("提示", data.error.message, "info");
                }
            });
...
var PolicyNodeClass = (function () {
    function init() {
        ...
        function loadData() {
            VE.Mask("");
            var url = webApi + "/ExamPolicyNode/Get?id=" + dataCache.id;
            nv.get(url, function (data) {
                VE.UnMask();
                if (data.success) {
                    setForm(data.result);
...
var PolicyItemClass = (function () {
    function init() {
        ...
        function getQuestionNum() {
            ...
            param.hardGrade = $("#hardGrade").combobox("getValue");
            param.labelIdList = $("#labelIdList").combotree("getValues");
            var url = webApi + "/QuestionBank/GetQuestionNum";
            nv.post(url, param, function (data) {
                if (data.success) {
...
var policyItemListClass = (function () {
    function init(policyId, policyItem) {
        var self = this;
        var queryParam = { policyId: policyId };
        ...
        this.policyNodeFormAddData = function (data) {
            var value = $("#policyNodeId2").combobox("getValue");
            data.push({ id:"00000000-0000-0000-0000-000000000000", policyNodeName: "全部"});
            $("#policyNodeId2").combobox("loadData", data);
            ...
            if (value != null) {
                ...
                if (hasValue) {
                    $("#policyNodeId2").combobox("setValue", value);
                }
            }
        };

        this.add = function (policyNode) {
            var questionType = {
                questionTypeUid: policyNode.questionTypeUid,
                questionTypeName: policyNode.questionTypeName
            };
            var rows = getNodeItemList(policyNode.id);
            var questionTotalNum = getItemListTotalQuestionNum(rows);
            policyItem.showAdd(rows, policyNode.id, questionType, questionTotalNum);
        };

        this.edit = function (index) {
            var row = $("#policyItemList-dg").datagrid("getRows")[index];
            $("#policyItemList-dg").datagrid("selectRow", index);
            var rows = getNodeItemList(row.policyNodeUid)
            var questionTotalNum = getItemListTotalQuestionNum(rows);
            policyItem.showEdit(rows, row, questionTotalNum);
        };
        ...
        this.del = function (index) {
            ...
            var rows;
            if (!$.isNumeric(index)) {
                var checkedRows = $("#policyItemList-dg").datagrid("getChecked");
                if (!checkedRows || checkedRows.length === 0) {
                    $.messager.alert("提示", "请先选择要删除的项！", "info");
                    return;
                }
            ...
            $.messager.confirm("删除确认", "确定进行删除操作吗？", function (b) {
                ...
                $("#policyItemList-dg").datagrid("loading");
                var url = webApi + "/ExamPolicyItem/Delete?ids=" + idArray.join(",");
                nv.get(url, function (data) {
                    $("#policyItemList-dg").datagrid("loaded");
                    if (data.success) {
                        $.messager.show({ title: "提示", msg: "删除成功" });
                        self.query();
                    } else {
        ...
        function loadData(param) {
            var url = webApi + "/ExamPolicyItem/GetAllList";
            $("#policyItemList-dg").datagrid("loading");
            nv.post(url,
                param,
                function (data) {
                    $("#policyItemList-dg").datagrid("loaded");
                    if (data.success) {
                        $("#policyItemList-dg").datagrid("loadData", data.result);
                    } else {
                        $.messager.alert("提示", data.error.message, "info");
                    }
                });
        ...
        function getParam() {
            queryParam.nodeId = $("#policyNodeId2").combobox("getValue");
            queryParam.questionBaseTypeCode = $("#questionBaseTypeCode2").combobox("getValue");
            if (stringIsEmpty(queryParam.nodeId)) {
                queryParam.nodeId = "00000000-0000-0000-0000-000000000000";
            }
            return queryParam;
        }

        function getNodeItemList(nodeId) {
            var rows = $("#policyItemList-dg").datagrid("getRows");
            var itemList = [];
            $.each(rows, function (k, v) {
                if (v.policyNodeUid === nodeId) {
                    itemList.push(v);
                }
            });
            return itemList;
        ...
        function getItemListTotalQuestionNum(itemList) {
            var num = 0;
