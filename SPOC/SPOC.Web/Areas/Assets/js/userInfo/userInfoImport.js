var ImportQuestionClass = (function () {
    //requiredControls 需要校验必填属性
    function init(fileType, successCallback, requiredControls) {
        ...
        function onFilesAdded(up, files) {
            if (requiredControls) {
                if (requiredControls.combotree("getValue") === "") {
                    $.messager.alert("提示","请选择班级再进行导入", "info");
                    return;
                }
            }
            if (!checkLogin()) {
          ...
          /*  if ($('#perList_div').form('validate') == false) {
            ...
            if (files.length > 0) {
                ...
                var url = "/User/Student/CreateStudentFromFile?departmentUid={0}";
                ...
                up.start();
                VE.Mask("正在导入");
                $("#userInfoImportDiv").window("close");
            }
        }
        ...
        var url = "/User/Student/CreateStudentFromFile";
        if (fileType === "excel") {
            initUploader({
                buttonId: "import-from-excel",
                url: url,
                mimeType: { title: "Office Excel", extensions: "xls" }
            });
        } else if (fileType === "word") {
            initUploader({
                buttonId: "import-from-word",
                url: url,
                mimeType: { title: "Office Word", extensions: "doc" }
            });
        }
    }
