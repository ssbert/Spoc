...
var NormalQuestion = (function() {
    ...
    function init(id, parentId) {
        ...
        this.onQuestionTypeChange = function(value) {
            ...
            if (value === "program" || value === "program_fill") {
                $("#standardAnswerLabel").text("输出结果：");
                ...
                //$("#questionAnalysisLabel").text("参考答案：");
            } else {
                $("#standardAnswerLabel").text("标准答案：");
                $("#questionAnalysisLabel").text("试题分析：");
            }
        }
                    ...
                    if (data.success) {
                        if (data.result.code === 0) {
                            if (data.result.result === $("#standardAnswer").textbox("getValue")) {
                                $("#run-result").textbox("setValue", "通过:\r\n输出:" + data.result.result);
                            } else {
                                $("#run-result").textbox("setValue", "不通过:\r\n输出:" + data.result.result);
                            }
            ...
            nv.post(url,
                ...
                function (data) {
                    if (data.success) {
                        ...
                        label = label.concat(data.result.label);
                        $("#label").val(label).trigger("change");
                        //剔除辅标签重复数据
                        var seclabel = $("#seclabel").val() == null ? [] : $("#seclabel").val();
                        $.each(label,
                            function (i, item) {
                                if ($.inArray(item, seclabel) > -1) {
                                    seclabel.splice($.inArray(item, seclabel), 1);
                                }
                            });
        ...
        function matchAnswerItem(matchText) {
            ...
            for (var i = 0; i < answerItems.length; i++) {
                items.push({ value: answerItems[i].value, ratio: answerItems[i].ratio });
            }
            //将匹配的文字加入得分点
            $.each(matchText,
                function (i, item) {
        ...
        function save (callback) {
            ...
            var url = serviceUrl + "QuestionBank/" + method;
            nv.post(url,
                param,
                function (data) {
                    VE.UnMask();
        ...
        function getFormParam() {
            ...
            if ($.inArray(param.questionBaseTypeCode, ["single", "multi"]) !== -1) {
            ...
            } else if ($.inArray(param.questionBaseTypeCode, ["answer", "operate", "program", "program_fill"]) !== -1) {
                var selectAnswers2 = [];
                ...
                if (param.questionBaseTypeCode === "program") {
                ...
                if (param.questionBaseTypeCode === "program_fill") {
                    param.param = $("#param").textbox("getValue");
                    param.inputParam = $("#inputParam").textbox("getValue");
                    param.multiTest = $("#multiTest").prop("checked");
            ...
            if (data.questionBaseTypeCode === "single") {
                ...
                if (data.questionBaseTypeCode === "program") {
                    ...
                    if (editor !== undefined && editor !== null) {
                        ...
                        setTimeout(function () { editor.setValue(data.standardCode); }, 1000);
                    }
        ...
        function getQuestionData(id, callback) {
            var url = serviceUrl + "QuestionBank/Get?id=" + id;
            VE.Mask("");
            nv.get(url,
                function(data) {
                    VE.UnMask();
        ...
        $(function () {
            ...
            category.getCategory(function() {
                if (stringIsEmpty(id)) {
                    self.onQuestionTypeChange("single");
                } else {
    ...
    function init(id) {
        ...
        this.onSaveAndNewlyAdd = function () {
            save(function (success) {
                ...
                window.location.href = "/QuestionBank/Manage/Edit";
            });
        };
            ...
            parent.$("#tabs")
                .tabs("add",
                {
                    ...
                    content: iframeHtml.format({
                        title: "新增子试题",
                        url: "/QuestionBank/Manage/Edit?parentId=" + dataCache.id
                    }),
                    closable: true,
                    icon: "icon-add"
                });
                ...
                function(k, v) {
                    parent.$("#tabs")
                        .tabs("add",
                        {
                            ...
                            content: iframeHtml.format({
                                title: "编辑子试题",
                                url: "/QuestionBank/Manage/Edit?id={0}&parentId={1}".format(v.id, dataCache.id)
                            }),
                            closable: true,
                            icon: "icon-edit"
                        });
        ...
        function getQuestionData() {
            var url = serviceUrl + "QuestionBank/Get?id=" + dataCache.id;
            VE.Mask("");
            nv.get(url,
                function(data) {
                    VE.UnMask();
        ...
        function getChildQuestionData(param) {
            var url = serviceUrl + "QuestionBank/GetChildPagination";
            nv.post(url, param,
                function(data) {
                    if (data.success) {
