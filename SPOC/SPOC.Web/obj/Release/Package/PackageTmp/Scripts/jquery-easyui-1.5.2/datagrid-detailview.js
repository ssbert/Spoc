		...
		for(var i=0; i<fields.length; i++){
			...
			if (col){
				...
				if (col.checkbox){
					...
					cc.push('<span class="datagrid-row-expander datagrid-row-expand" style="display:inline-block;width:16px;height:16px;cursor:pointer;" title="展开详细信息"/>');
					//cc.push('</div>');
	...
	expandRow: function(jq, index){
		return jq.each(function(){
			...
			if (expander.hasClass('datagrid-row-expand')){
			    expander.removeClass('datagrid-row-expand').addClass('datagrid-row-collapse').attr("title","关闭详细信息");
				var tr1 = opts.finder.getTr(this, index, 'body', 1).next();
				var tr2 = opts.finder.getTr(this, index, 'body', 2).next();
				tr1.show();
				tr2.show();
	...
	collapseRow: function(jq, index){
		return jq.each(function(){
			...
			if (expander.hasClass('datagrid-row-collapse')){
			    expander.removeClass('datagrid-row-collapse').addClass('datagrid-row-expand').attr("title", "展开详细信息");
				var tr1 = opts.finder.getTr(this, index, 'body', 1).next();
				var tr2 = opts.finder.getTr(this, index, 'body', 2).next();
				tr1.hide();
				tr2.hide();
