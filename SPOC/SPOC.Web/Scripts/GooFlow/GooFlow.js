...
function GooFlow(bgDiv,property){
    ...
    if (property.initLabelText) {
        this.$title = property.initLabelText;
    }
	...
	if(property.haveHead){
      ...
      tmp+="<label title='"+(property.initLabelText||"newFlow_1")+"' "
        ...
        for (var x = 0; x < property.headBtns.length; ++x) {
            var title = GooFlow.prototype.remarks.headBtns[property.headBtns[x]];
            var titleText = title ? " title='" + title+"'":"";
			tmp+="<a href='javascript:void(0)' class='GooFlow_head_btn'"+titleText+"><i class='ico_"+property.headBtns[x]+"'></i>"+title+"</a>";
		}
		tmp+="</div>";
		this.$head=$(tmp);
	    ...
	    this.onBtnEditClick = null;//编辑按钮定义（自定义添加的）
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
                ...
                case "ico_edit":    if (This.onBtnEditClick != null) This.onBtnEditClick(); break;
			}
		});
	}
	var toolWidth=0;
...
GooFlow.prototype={
	...
	getSvgMarker:function(id,color){
		...
		m.setAttribute("viewBox","0 0 0 0");
		m.setAttribute("refX",0);
		m.setAttribute("refY",0);
		m.setAttribute("markerUnits","strokeWidth");
		m.setAttribute("markerWidth",0);
		m.setAttribute("markerHeight",0);
