    ...
    this.GetUserExamAnswerXML = function (examGradeUid) {
        ...
        try {
            ...
            for (var i = 0; i < length; i++) {
                ...
                inOrder = eval($("inOrder_" + questionUid).value);
                ...
                //改版后遗漏了判断题开启评卷策略“答对得分,不答不得分,答错扣分”的情况 Lopping 2012-09-28
                    ...
                    if (this.FilterQuestionAnswerForFill(standardAnswer) == this.FilterQuestionAnswerForFill(answer_text)) {
                        ...
                        if (inOrder == "false" || inOrder == false) {
                            //非顺序匹配
                            var count = arrStandardAnswer.length;
                            var rightCount = 0;
                            if (count == arrUserAnswer.length) {
                                
                                for (j = 0; j < arrUserAnswer.length; j++) {

                                    var hasAnswer = false;
                                    for (var l = 0; l < arrStandardAnswer.length; l++) {
                                        //判断一个空是否有多个答案
                                        var isMultipleAnswers = false;
                                        if (arrStandardAnswer[l].toLowerCase().indexOf("&") > -1) {
                                            isMultipleAnswers = true;
                                        }

                                        if (this.FilterQuestionAnswerForFill(arrUserAnswer[j]) == this.FilterQuestionAnswerForFill(arrStandardAnswer[l]) && !isMultipleAnswers) {
                                            hasAnswer = true;
                                        }
                                        else {
                                            //如果不是完全配配，则还要看是否是几个中任一个都行,中间用&分隔
                                            if (isMultipleAnswers) {
                                                var arrOneAnswer = arrStandardAnswer[l].split('&');
                                        ...
                                        if (hasAnswer) {
                                            arrStandardAnswer.splice(l, 1);//删掉已匹配答案
                                            break;
                                        }
                                    }
