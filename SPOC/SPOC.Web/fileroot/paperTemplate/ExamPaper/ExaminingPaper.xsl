...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 xmlns:user="http://www.wrox.com.uuid">
  ...
  <xsl:param name="file_server_web_root_path"></xsl:param>
  ...
  <msxsl:script language="javascript" implements-prefix="user">
    ...
    function ReplaceFillInContent(Content,questionUid,fileWebRootPath)
    {
	  Content = GetQuestionFilePath(Content,fileWebRootPath);
      ...
      while (lowLinePos > -1)
      {
          ...
          if (lowLinePos != preLowLinePos + 1)
          {
              ...
              if (lowLineCount >= 3)
              {
                  oneInputBoxString = "<input type='text' class='Nsb_exam_tiankong' id='Answer_" + questionUid + "' name='Answer_" + questionUid + "'  maxlength='1950' class='LineText' style='overflow:visible;width:" + (lowLineCount * 30) + "' value='" + oneAnswer + "' onkeyup=\"CheckFillQuestionAnswerLength(this)\" onchange=\"SetQuestionAnswerStatus('"+questionUid+"',true)\" />";
      ...
      questionTextHtml=questionTextHtml+"<span id='spanLoadQuestionText"+questionUid+"' class='Nsb_exam_refresh' title='重新加载题干' onclick=\"javascript:LoadQuestionText('"+questionUid+"')\">&nbsp;&nbsp;&nbsp;&nbsp;</span>";
    ...
    function GetQuestionText(questionText,questionUid,fileWebRootPath)
    {
      var questionTextHtml="";
	  questionText = GetQuestionFilePath(questionText,fileWebRootPath);
      questionTextHtml="<span id='spanQuestionText"+questionUid+"' name='spanQuestionText"+questionUid+"' >";
    ...
    {
      _navigatorIndex=1;
      _navigatorHtml=_navigatorHtml+"<div class='Nsb_exam_title'>"+Arabia_to_Chinese(paperNodeIndex)+"、"+paperNodeName+"</div>";
      //生成表格头
      _navigatorHtml=_navigatorHtml+"<div><table width='242' border='0' cellspacing='1' cellpadding='0'>";
    ...
    {
      //补全Td
      for(;_navigatorIndex<=7;_navigatorIndex++)
      {
        _navigatorHtml=_navigatorHtml+"<td width='35px' height='37px'></td>";
      }
    ...
    function AddQuestionToNavigator(questionUid,paperNodeIndex,parenntQuestionIndex,questionIndex,questionIndexView)
    {
      ...
      {
        if(_navigatorIndex!=1 && _navigatorIndex%7==1)
        {
          _navigatorHtml=_navigatorHtml+"</tr><tr>";
          _navigatorIndex=1;
        }
	      _navigatorHtml=_navigatorHtml+"<td width='35px' height='37px' onclick=\"javascript:iframePaper.GoToQuestion('"+paperNodeIndex+"','"+parenntQuestionIndex+"','"+questionIndex+"')\" id='tdQuestionVavigator_"+questionUid+"' class='Nsb_exam_nav_no_flag_no_answer' align='center'><a id='lnkVavigator_"+questionUid+"' title='"+paperNodeIndex+"_"+parenntQuestionIndex+"_"+questionIndex+"'>"+questionIndexView+"</a></td>";
	...
	function GetQuestionFilePath(questionText,fileWebRootPath){
		questionText=questionText.replace(/<PARAM NAME="SRC" VALUE=".\//gi,"<PARAM NAME=\"SRC\" VALUE=\""+fileWebRootPath);
		questionText=questionText.replace(/<PARAM NAME="URL" VALUE=".\//gi,"<PARAM NAME=\"URL\" VALUE=\""+fileWebRootPath);
		questionText=questionText.replace(/<PARAM NAME="MOVIE" VALUE=".\//gi,"<PARAM NAME=\"MOVIE\" VALUE=\""+fileWebRootPath);
	
		questionText=questionText.replace(/src=".\//gi,"src=\""+fileWebRootPath);
		questionText=questionText.replace(/src='.\//gi,"src='"+fileWebRootPath);
		questionText=questionText.replace(/src=.\//gi,"src="+fileWebRootPath);
	
		questionText=questionText.replace(/href=".\//gi,"href=\""+fileWebRootPath);
		questionText=questionText.replace(/href='.\//gi,"href='"+fileWebRootPath);
		questionText=questionText.replace(/href=.\//gi,"href="+fileWebRootPath);
	
		if(questionText.indexOf("flv")){
			questionText=questionText.replace(/<source>.\/..\/..\//gi,"<source>"+fileWebRootPath);
		}
		else{
			questionText=questionText.replace(/<source>.\//gi,"<source>"+fileWebRootPath);
		}
  ...
  <xsl:template match = "exam_paper_object">
      ...
      <xsl:choose>
        <xsl:when test="$exam_do_mode_code='question'">
          <TABLE BORDER="0" cellpadding="0" cellspacing="0" style="margin-left:40px;margin-top:15px;">
                <Tr>
                    <td>
                        <input type="button" class="Nsb_exam_operate_btn" name="btnPreQuestion" id="btnPreQuestion"  onclick="GoToPreQuestion()"  value="上一题"/></td>
                    <td>
                        &#160;&#160;&#160;&#160;</td>
                    <td>
                        <input type="button" class="Nsb_exam_operate_btn" name="btnNextQuestion" id="btnNextQuestion" onclick="GoToNextQuestion()"  value="下一题"/></td>
                </Tr>
            </TABLE>
        </xsl:when>
      </xsl:choose>
  ...
  <xsl:template name="exam_paper_node">
    <xsl:param   name="question_index" select="'0'" />
    ...
    <xsl:element name="div">
      ...
      <xsl:attribute name="class">Nsb_exam_sttitle</xsl:attribute>
      <xsl:attribute name="title">展开/收缩</xsl:attribute>
      <xsl:attribute name="onclick">jscomFlexObject(document.getElementById('tblPaperNode_<xsl:value-of select="$paper_node_index"/>'))</xsl:attribute>
    ...
    <xsl:element name="div">
      ...
      <xsl:attribute name="class">Nsb_exam_stcontent</xsl:attribute>
      <xsl:attribute name="style">
        <xsl:choose>
          <xsl:when test="$exam_do_mode_code='paper'">display:block</xsl:when>
          <xsl:otherwise>display:none</xsl:otherwise>
                ...
                <TR  >
                  <xsl:element name="td">
                    ...
                    <xsl:attribute name="style">
                     ...
                     <xsl:element name="table">
                     <TR class="Nsb_exam_sttitle_small" id='panelQuestionText_'>
                      <xsl:element name="td">
                        <xsl:attribute name="id">
                          panelQuestionText_<xsl:value-of select="question_uid"/>
                        </xsl:attribute>
                        ...
                        <xsl:element name="input">
                          ...
                          <xsl:attribute name="value">
                            <xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                        </xsl:element>
                        <!-- //QuestionUid -->
                        ...
                        <xsl:element name="input">
                          <xsl:attribute name="type">hidden</xsl:attribute>
                          <xsl:attribute name="id">
                            hidQuestionBaseTypeCode_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            hidQuestionBaseTypeCode_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="value">
                            <xsl:value-of select="question_base_type_code"/>
                          </xsl:attribute>
                        </xsl:element>
                        <!-- //question_base_type_code -->
                        ...
                        <xsl:element name="input">
                          <xsl:attribute name="type">hidden</xsl:attribute>
                          <xsl:attribute name="id">
                            hidQustionTypeUid_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            hidQustionTypeUid_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="value">
                            <xsl:value-of select="question_type_uid"/>
                          </xsl:attribute>
                        </xsl:element>
                        <!-- //question_type_uid -->
                        ...
                        <xsl:element name="input">
                          <xsl:attribute name="type">hidden</xsl:attribute>
                          <xsl:attribute name="id">
                            hidStandardAnswer_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            hidStandardAnswer_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="value">
                            <xsl:value-of select="standard_answer"/>
                          </xsl:attribute>
                        </xsl:element>
                        <!-- //standard_answer -->
                        ...
                        <xsl:element name="input">
                          <xsl:attribute name="type">hidden</xsl:attribute>
                          <xsl:attribute name="id">
                            inOrder_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            inOrder_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="value">
                            <xsl:value-of select="in_order"/>
                          </xsl:attribute>
                        </xsl:element>
                        <!-- //in_order -->
                        ...
                        <xsl:element name="input">
                          <xsl:attribute name="type">hidden</xsl:attribute>
                          <xsl:attribute name="id">
                            hidPaperQuestionScore_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            hidPaperQuestionScore_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="value">
                            <xsl:value-of select="paper_question_score"/>
                          </xsl:attribute>
                        </xsl:element>
                        <!-- //paper_question_score -->
                        ...
                        <xsl:element name="input">
                          <xsl:attribute name="type">hidden</xsl:attribute>
                          <xsl:attribute name="id">
                            hidSelectAnswerScore_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            hidSelectAnswerScore_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="value">
                            <xsl:value-of select="select_answer_score"/>
                          </xsl:attribute>
                        </xsl:element>
                        <!-- //select_answer_score -->
                        ...
                        <xsl:element name="input">
                          <xsl:attribute name="type">hidden</xsl:attribute>
                          <xsl:attribute name="id">
                            hidPaperQuestionExamTime_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            hidPaperQuestionExamTime_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="value">
                            <xsl:value-of select="paper_question_exam_time"/>
                          </xsl:attribute>
                        </xsl:element>
                        <!-- //is_answer_by_html -->
                        ...
                        <xsl:element name="input">
                          <xsl:attribute name="type">hidden</xsl:attribute>
                          <xsl:attribute name="id">
                            hidIsAnswerByHtml_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            hidIsAnswerByHtml_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="value">
                            <xsl:value-of select="is_answer_by_html"/>
                          </xsl:attribute>
                        </xsl:element>
                        <!-- //is_answer_by_html -->
                        ...
                        <xsl:element name="input">
                          <xsl:attribute name="type">hidden</xsl:attribute>
                          <xsl:attribute name="id">
                            hidOperateTypeCode_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            hidOperateTypeCode_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="value">
                            <xsl:value-of select="operate_type_code"/>
                          </xsl:attribute>
                        </xsl:element>
                        <!-- //operate_type_code -->
                        ...
                        <xsl:element name="input">
                          <xsl:attribute name="type">hidden</xsl:attribute>
                          <xsl:attribute name="id">
                            hidUserAnswerTime_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            hidUserAnswerTime_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                        ...
                        <!--<xsl:element name="span">
                          <xsl:attribute name="onclick">
                            javascript:SetQuestionBookmark(<xsl:value-of select="$paper_node_index"/>,<xsl:value-of select="$parent_question_index"/>,<xsl:value-of select="$question_index"/>)
                          </xsl:attribute>
                          <xsl:attribute name="id">
                            spanQuestionMark_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="class">Nsb_exam_no_flag</xsl:attribute>
                        ...
                        <xsl:element name="input">
                          <xsl:attribute name="id">
                            hidIsRead_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            hidIsRead_<xsl:value-of select="question_uid"/>
                          </xsl:attribute>
                        ...
                        <xsl:element name="input">
                          <xsl:attribute name="id">
                            hidQuestion_<xsl:value-of select="$paper_node_index"/>_<xsl:value-of select="$parent_question_index"/>_<xsl:value-of select="$question_index"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            hidQuestion_<xsl:value-of select="$paper_node_index"/>_<xsl:value-of select="$parent_question_index"/>_<xsl:value-of select="$question_index"/>
                          </xsl:attribute>
                          <xsl:attribute name="type">hidden</xsl:attribute>
                          <xsl:attribute name="value">
                            <xsl:value-of select="string(question_uid)"/>
                          </xsl:attribute>
                        </xsl:element>
                        <!--<xsl:element name="a">
                          <xsl:attribute name="id">
                            lnkQuestion_<xsl:value-of select="$paper_node_index"/>_<xsl:value-of select="$parent_question_index"/>_<xsl:value-of select="$question_index"/>
                          </xsl:attribute>
                          <xsl:attribute name="name">
                            lnkQuestion_<xsl:value-of select="$paper_node_index"/>_<xsl:value-of select="$parent_question_index"/>_<xsl:value-of select="$question_index"/>
                          </xsl:attribute>
                        ...
                        <xsl:choose>
                          <xsl:when test="question_base_type_code='fill'">
                            <xsl:value-of select="user:ReplaceFillInContent(string(question_text),string(question_uid),string($file_server_web_root_path))" disable-output-escaping="yes"/>
                          </xsl:when>
                          <xsl:when test="question_base_type_code='typing'">
                            <!-- 打字题不要出内容 -->
                          </xsl:when>
                          <xsl:otherwise>
                            <xsl:value-of select="user:GetQuestionText(string(question_text),string(question_uid),string($file_server_web_root_path))" disable-output-escaping="yes"/>
                          </xsl:otherwise>
                        </xsl:choose>
                        <xsl:if test="$is_show_score='Y'">
                            ...
                            <xsl:if test="$exam_do_mode_code='question' and $is_need_limited_time='Y'">
                              (<font color="red">
                                <xsl:element name="label">
                                  <xsl:attribute name="id">
                                    divQuestionTimePanel_<xsl:value-of select="question_uid"/>
                                  </xsl:attribute>
                                  <xsl:text disable-output-escaping="yes"></xsl:text>
                                </xsl:element>
                              </font>秒)
                    ...
                    <xsl:for-each select="sub_exam_paper_node_questions/exam_paper_node_question">
                      ...
                      <TR>
                        <td align="left">
                          <xsl:element name="table">
                            <xsl:attribute name="id">
                              tblQuestion_<xsl:value-of select="question_uid"/>
                            </xsl:attribute>
                            <xsl:attribute name="width">100%</xsl:attribute>
                            <xsl:value-of select="user:AddCurrentGloableQuestionIndex()"/>
                            <!--试题计算加1-->
                            <xsl:call-template name="exam_paper_node_question">
                              <xsl:with-param name="paper_node_index" select="$paper_node_index"></xsl:with-param>
                              <xsl:with-param name="parent_question_index" select="$parent_question_index"></xsl:with-param>
                              <xsl:with-param name="question_index" select="string(position())"></xsl:with-param>
                              <xsl:with-param name="question_index_view" select="user:AddString(string(position()),')')"></xsl:with-param>
                              <!--试题按每大题编号时用-->
                              <!--<xsl:with-param name="question_index_view" select="user:GetCurrentGloableQuestionIndex()"></xsl:with-param>-->
                              <!--试题顺序号整卷编号时用-->
                            </xsl:call-template>
                            <!-- 添加试题导航信息 -->
                            <xsl:value-of select="user:AddQuestionToNavigator(string(question_uid),$paper_node_index,$parent_question_index,string(position()),user:AddString(string(position()),')'))"/>
                            <!--试题按每大题编号时用-->
                            <!-- <xsl:value-of select="user:AddQuestionToNavigator(string(question_uid),$paper_node_index,$parent_question_index,string(position()),string(user:GetCurrentGloableQuestionIndex()))"/> -->
                            <!--试题顺序号整卷编号时用-->
                          </xsl:element>
                        </td>
  ...
  <xsl:template name="exam_paper_node_question">
      ...
      <TR class="Nsb_exam_sttitle_small" id='panelQuestionText_' >
		  <xsl:element name="td">
			  <xsl:attribute name="id">panelQuestionText_<xsl:value-of select="question_uid"/></xsl:attribute>
        ...
        <xsl:element name="input">
          <xsl:attribute name="type">hidden</xsl:attribute>
          <xsl:attribute name="id">inOrder_<xsl:value-of select="question_uid"/></xsl:attribute>
          <xsl:attribute name="name">inOrder_<xsl:value-of select="question_uid"/></xsl:attribute>
          <xsl:attribute name="value"><xsl:value-of select="in_order"/></xsl:attribute>
        </xsl:element>
        <!-- //in_order -->
			  ...
			  <xsl:element name="img">
				...
				<xsl:attribute name="src">css/image/examattend/flag2.gif</xsl:attribute>
				<xsl:attribute name="title">标注/取消标注</xsl:attribute>
				<xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;</xsl:text>
			  </xsl:element>
			  ...
			  <xsl:choose>
				<xsl:when test="paper_question_exam_time!='0'">
				  <xsl:if test="$exam_do_mode_code='question' and $is_need_limited_time='Y'">
					  (<font color="red">
						<xsl:element name="label">
								<xsl:attribute name="id">divQuestionTimePanel_<xsl:value-of select="question_uid"/></xsl:attribute>
								<xsl:text disable-output-escaping="yes"></xsl:text>
							  </xsl:element>
					  </font>秒)
				  </xsl:if>
        ...
        <TD>
          ...
          <xsl:choose>
            <xsl:when test="question_base_type_code='single' or question_base_type_code='eva_single'">
              <!-- 单选题 -->
              <table class="Nsb_exam_xuanxiang">
              ...
              <tr>
                ...
                <xsl:element name="input">
                  ...
                  <xsl:element name="span">
                    ...
                    <xsl:value-of select="user:GetQuestionFilePath(string(select_answer_text),string($file_server_web_root_path))"  disable-output-escaping="yes"/>
                  </xsl:element>
                </td>
                </tr>
              </xsl:for-each>
            ...
            <xsl:when test="question_base_type_code='judge_correct'">
              ...
              <xsl:element name="TextArea">
                ...
                <xsl:attribute name="class">Nsb_exam_textarea</xsl:attribute>
            ...
            <xsl:when test="question_base_type_code='answer'">
			  ...
			  <xsl:when test="$exam_is_only_upload_file='Y' and is_answer_by_html='Y' and is_only_upload_file!='N'">
				  ...
				  <xsl:element name="input">
					...
					<xsl:attribute name="class">Nsb_exam_operate_btn</xsl:attribute>
			  ...
			  <xsl:otherwise>
				  ...
				  <xsl:if test="is_answer_by_html='Y'">
					  <xsl:element name="input">
						  ...
						  <xsl:attribute name="id">lnkAnswerByHtml_<xsl:value-of select="question_uid"/>
						  </xsl:attribute>
						  <xsl:attribute name="class">Nsb_exam_operate_btn</xsl:attribute>
						  <xsl:attribute name="onclick">javascript:OpenHtmlEditor('Answer_<xsl:value-of select="question_uid"/>','ExamAnswer/Exam_<xsl:value-of select="$exam_uid"/>/ExamGrade_<xsl:value-of select="$exam_grade_uid"/>/<xsl:value-of select="question_uid"/>')
						  </xsl:attribute>
					  </xsl:element>
				  </xsl:if>
			  </xsl:otherwise>
            ...
            <xsl:when test="question_base_type_code='program'">
              <!--编程题-->
              <xsl:element name="div">
                <xsl:attribute name="id">Answer_<xsl:value-of select="question_uid"/></xsl:attribute>
                <xsl:attribute name="name">Answer_<xsl:value-of select="question_uid"/></xsl:attribute>
                <xsl:attribute name="class">code_editor</xsl:attribute>
                <xsl:attribute name="style">width:800px;height:600px;border:1px solid grey;</xsl:attribute>
                <xsl:attribute name="language"><xsl:value-of select="language"/></xsl:attribute>
                <xsl:attribute name="type_code">program</xsl:attribute>
              </xsl:element>
            </xsl:when>
            
            <xsl:when test="question_base_type_code='program_fill'">
              <!--编程填空题-->
              <xsl:element name="div">
                <xsl:attribute name="id">Answer_<xsl:value-of select="question_uid"/></xsl:attribute>
                <xsl:attribute name="name">Answer_<xsl:value-of select="question_uid"/></xsl:attribute>
                ...
                <xsl:attribute name="style">width:800px;height:600px;border:1px solid grey;</xsl:attribute>
                <xsl:attribute name="language"><xsl:value-of select="language"/></xsl:attribute>
                <xsl:attribute name="type_code">program_fill</xsl:attribute>
              </xsl:element>
              <xsl:element name="div">
                <xsl:attribute name="id">preinstallCode_<xsl:value-of select="question_uid"/></xsl:attribute>
                <xsl:attribute name="name">preinstallCode_<xsl:value-of select="question_uid"/></xsl:attribute>
                <xsl:attribute name="style">display:none;</xsl:attribute>
                <xsl:attribute name="language"><xsl:value-of select="language"/></xsl:attribute>
                <xsl:value-of select="preinstall_code"/>
              </xsl:element>
            </xsl:when>
            ...
            <xsl:when test="question_base_type_code='operate'">
              ...
              <xsl:element name="input">
                ...
                <xsl:attribute name="class">Nsb_exam_operate_btn</xsl:attribute>
                <xsl:attribute name="onclick">OpenContractTextFile('<xsl:value-of select="$exam_grade_uid"/>','<xsl:value-of select="question_uid"/>','<xsl:value-of select="$exam_uid"/>','<xsl:value-of select="operate_type_code"/>')</xsl:attribute>
              </xsl:element>
              <xsl:element name="input">
                <xsl:attribute name="id">Answer_<xsl:value-of select="question_uid"/></xsl:attribute>
                <xsl:attribute name="name">Answer_<xsl:value-of select="question_uid"/></xsl:attribute>
              ...
              <xsl:element name="input">
                ...
                <xsl:attribute name="value"><xsl:value-of select="question_text"/></xsl:attribute>
                <xsl:attribute name="type">hidden</xsl:attribute>
              </xsl:element>
            </xsl:when>
