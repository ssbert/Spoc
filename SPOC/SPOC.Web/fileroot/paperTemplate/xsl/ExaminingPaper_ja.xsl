 ...
 xmlns:user="http://www.wrox.com.uuid">
  ...
  <msxsl:script language="javascript" implements-prefix="user">
    ...
    function AddPaperNodeToNavigator(paperNodeIndex)
    {
      ...
      _navigatorHtml=_navigatorHtml+"<span style='font-size:12px;'><b>第"+Arabia_to_Chinese(paperNodeIndex)+"大题</b></span>";
      //生成表格头
    ...
    <TABLE BORDER="0" WIDTH="98%" align="center"  cellpadding="0" cellspacing="0">
      ...
      <TR>
        <TD align="left" valign="bottom" wrap="false" COLSPAN="2">
          ...
          共<xsl:value-of select="exam_paper/total_score"></xsl:value-of>点
        </TD>
      </TR>
          ...
          <TR>
            <TD colspan="2" valign="top">
              <TABLE BORDER="0" cellpadding="0" cellspacing="0">
					<Tr>
						<td>
							<input type="button" class="Button" name="btnPreQuestion" id="btnPreQuestion"  onclick="GoToPreQuestion()"  value="上一题"/></td>
						<td>
							&#160;&#160;&#160;&#160;</td>
						<td>
							<input type="button" class="Button" name="btnNextQuestion" id="btnNextQuestion" onclick="GoToNextQuestion()"  value="下一题"/></td>
					</Tr>
              	</TABLE>
            </TD>
          </TR>
        ...
        <font color="white">
		  ...
		  <xsl:if test="$is_show_score='Y'">
			  <xsl:if test="question_score>0">
				，每题<xsl:value-of select="question_score"></xsl:value-of>点
			  </xsl:if>
			  ，总共<xsl:value-of select="total_score"></xsl:value-of>点
		  </xsl:if>
		  ）
  ...
  <xsl:template name="exam_paper_node_question">
      ...
      <TR bgcolor="#CCCCCC" height="30px">
        <td align="left" width="100%" class="PaperQuestionTextBackColor">
          ...
          <xsl:if test="$is_show_score='Y'">
            （<xsl:value-of select="paper_question_score"></xsl:value-of> 点）
          </xsl:if>
          <!-- //QuestionText -->
        </td>
      </TR>
