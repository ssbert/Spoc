...
namespace SPOC.Core
{
    /*抽离挑战题导入
    */
    public class ImportChallengeBaseHelper
    {
        ...
        protected readonly IRepository<ChallengeQuestion, Guid> _iChallengeQuestionRep;
        ...
        /// <param name="iChallengeQuestionRep"></param>
        /// <param name="iExamQuestionTypeRep"></param>
        /// <param name="iUnitOfWorkManager"></param>
        /// <exception cref="UserFriendlyException"></exception>
        public ImportChallengeBaseHelper(string rootPath, Guid folderUid, IRepository<NvFolder, Guid> iNvFolderRep, IRepository<ChallengeQuestion, Guid> iChallengeQuestionRep, IRepository<ExamQuestionType, Guid> iExamQuestionTypeRep, IUnitOfWorkManager iUnitOfWorkManager, IRepository<QuestionStandardCode, Guid> iQuestionStandardCodeRep,string language, string newMoocApiUrl, IRepository<Label, Guid> iLabelRep, IRepository<QuestionLabel, Guid> iQuestionLabelRep)
        {
        ...
        /// <summary>
        /// 把本文内容转化为ChallengeQuestionRow对像并保存在数据中
        /// </summary>
            ...
            Hashtable publicProperty, Guid operationUid)
        {
            ...
            bool isNew;
            //取当前属性的QuestionUid,然后创建ChallengeQuestionRow对像,如果没有新添加.
            ChallengeQuestion quRow;
            var qUid = Guid.Empty;
            var questionType = "";
            var cFolder = "";
            ...
            if (qUid == Guid.Empty)
            {
                quRow = new ChallengeQuestion { Id = Guid.NewGuid() };
                isNew = true;
            }
            else
            {
                quRow = _iChallengeQuestionRep.Get(qUid);
                isNew = false;
                if (quRow == null)
                {
                    quRow = new ChallengeQuestion { Id = qUid };
                    isNew = true;
                }
            }
            //取本试题的题型
            ...
            if (!questionType.Contains("编程"))
            {
                reValue.Message = "不支持题型【"+ questionType + "】";
            ...
            else
            {
                ...
                else
                {
                    ...
                    cFolderUid = GetFolderUid("challenge_cpp", parentUidGuid, cFolder, operationUid, true);
                }
            }
            //设置ChallengeQuestionRow对像的属性
            foreach (string pkey in properties.Keys)
            {
                ...
                else
                {
                    ...
                    else if (pkey == questionField["standardAnswer"])
                    {
                        ...
                        sdAnswer = GetStandardAnswerText(quRow.questionBaseTypeCode, sdAnswer);
            ...
            {
                quRow.listOrder = _iChallengeQuestionRep.Count() == 0
                    ? 0
                    : _iChallengeQuestionRep.GetAll().Max(a => a.listOrder) + 1;
            }
            //编号为空时
            ...
            //默认难度
            if (string.IsNullOrEmpty(quRow.hardGrade))
            quRow.hardGrade = "1";
            ...
            if (!success)
            {
                reValue.Message = "编程题云端存储保存失败!请联系管理员";
                reValue.HasError = true;
                return reValue;
            }
            //插入前先检查是否存在相同试题
            var row = quRow;
            var sameQuestions = _iChallengeQuestionRep.GetAll()
            ...
            if (!sameQuestions.Any()) //相同分类下不存在相同试题则导入
            {
                ...
                _iChallengeQuestionRep.Insert(quRow);
            ...
            else
            {
                ...
                {
                    quRow.score = score;
                    _iChallengeQuestionRep.Update(quRow);
                    _iUnitOfWorkManager.Current.SaveChanges();
                }
            }
            if (reValue.HasError)
            ...
            {
                var parentQuestion = _iChallengeQuestionRep.Get(quRow.parentQuestionUid);
                var score =
                    _iChallengeQuestionRep.GetAll()
                        .Where(a => a.parentQuestionUid == quRow.parentQuestionUid)
                        .Sum(a => a.score);
                parentQuestion.score = score;
                _iChallengeQuestionRep.UpdateAsync(parentQuestion);
            }
            ...
            {
                _iChallengeQuestionRep.Delete(quRow.Id);
        ...
        private  bool CompileQuestionPublish(QuestionToCloudDto question)
        {
            ...
            var apiContent = System.Web.HttpUtility.UrlEncode(jsonValue.ToString(),
                Encoding.UTF8);
            var url = _newMoocApiUrl + "/lib/question?sign=";
            //同步调用异步方法
            var result = AsyncHelper.RunSync(
                () => HttpHelper.PostResponseSerializeData<ApiResponseResult<dynamic>>(url, apiContent));
        ...
        /// 通过字段名进行赋值
        /// </summary>
        private ChallengeQuestion SetPropertity(string fieldName, object propertityValue, ChallengeQuestion eq)
        {
            switch (fieldName)
            {
                case "id":
                    ...
                    break;
                case "title":
                    eq.title = (string)propertityValue;
                    break;
                case "folderUid":
