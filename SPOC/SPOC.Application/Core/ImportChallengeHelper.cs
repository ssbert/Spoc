...
namespace SPOC.Core
{
     /*抽离挑战题导入
      */
    internal class ImportChallengeHelper:ImportChallengeBaseHelper
    {
        
        
        public ImportChallengeHelper(string rootPath, Guid folderUid, IRepository<NvFolder, Guid> iNvFolderRep, IRepository<ChallengeQuestion, Guid> iChallengeQuestionRep, IRepository<ExamQuestionType, Guid> iExamQuestionTypeRep, IUnitOfWorkManager iUnitOfWorkManager, IRepository<QuestionStandardCode, Guid> iQuestionStandardCodeRep,string language, string newMoocApiUrl, IRepository<Label, Guid> iLabelRep, IRepository<QuestionLabel, Guid> iQuestionLabelRep) : base( rootPath, folderUid, iNvFolderRep, iChallengeQuestionRep, iExamQuestionTypeRep, iUnitOfWorkManager, iQuestionStandardCodeRep, language, newMoocApiUrl, iLabelRep,iQuestionLabelRep)
        {
        }
            ...
            while (nPos > -1)
            {
                try
                {
                         ...
                         sOneContent.IndexOf("]：", StringComparison.Ordinal) >= 0))
                    {
                        ...
                        else if (sPropertyValue == "标题")
                        {
                            if (publicProperty.ContainsKey("title"))
                            {
                                publicProperty["title"] = sPropertyValue;
                            }
                            else
                            {
                                publicProperty.Add("title", sPropertyValue);
                            }
                        }
                    ...
                    else if (sOneContent != "" && !sOneContent.StartsWith("//"))
                    {
                        ...
                        {
                            errMsg = errMsg + string.Format(("[错误：]第{0}题导入失败:"), (i + 1)) + retValue.Message +
                                     Environment.NewLine;
                            errCount += 1;
                        }
                        else
                        {
                            ChallengeQuestion quRow = (ChallengeQuestion) retValue.GetValue("question");
                            ...
                            if (retValue.HasError == false)
                            {
                                ...
                                if (retValueCopyFile.HasError)
                                {
                                    ...
                                    {
                                        errMsg = errMsg + string.Format(("[错误：]第{0}题导入试题内容成功，但处理文件失败:"), (i + 1)) +
                                                 retValueCopyFile.Message + Environment.NewLine;
                                    }
                                }
                                else
                                {
                                ...
                                if (hasCopyContentFile || hasCopySelectAnswerFile || hasCopyStandardAnswerFile || hasCopyQuestionAnslysisFile)
                                {
                                    ...
                                    _iChallengeQuestionRep.UpdateAsync(quRow);
                                }
        ...
        public ReturnValue CheckQuestionTextFormat(string sOneContent, Dictionary<string, string> questionField, Hashtable publicProperty, int questionIndex)
        {
            ...
            foreach (string pkey in properties.Keys)
            {
                ...
                if (pkey == questionField["questionCode"])
                {
                    ...
                    var entity = _iChallengeQuestionRep.FirstOrDefault(p => p.questionCode == code);
                    if (entity != null)
                    {
